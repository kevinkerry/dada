var windowsArr = [];
		...
		function autoSearch() {
		    ...
		    var auto;
		    var defaultCity="";
		    var province = "广东省";//document.getElementById("province").value;
		    var city = "广州市";//document.getElementById("city").value;
		    ...
		    if (tipArr&&tipArr.length>0) {                 
		        for (var i = 0; i < tipArr.length; i++) {
		            resultStr += "<div id='divid" + (i + 1) + "' onmouseover='openMarkerTipById(" + (i + 1)
		                        ...
		                        + ",this)' style=\"font-size: 13px;cursor:pointer;padding:5px 5px 5px 5px;\"" + "data=" + tipArr[i].adcode + ">" + tipArr[i].name + "<span style='color:#C1C1C1;'>"+ tipArr[i].district + "</span>" +
		                        		"<input type='hidden'  id = 'inid"+(i + 1)+"' value="+tipArr[i].district+tipArr[i].name+" /></div>";
		        }
		    }
		    else  {
		...
		function selectResult(index) {
		   ...
		   // var text = document.getElementById("divid" + (index + 1)).innerHTML.replace(/<[^>].*?>.*<\/[^>].*?>/g,"");
		    var text = document.getElementById("inid" + (index + 1)).value;
		...
		function getLnglat(e){ 
			regeocoder(e.lnglat.getLat(),e.lnglat.getLng());
		...
		function addMarker_location(){
		 ...
		 //逆地理编码 
		 function regeocoder(x,y) {
			 var lnglatXY = [y,x];
		        var mapgeocoder = new AMap.Geocoder({
		            radius: 1000,
		            extensions: "all"
		        });        
		        mapgeocoder.getAddress(lnglatXY, function(status, result) {
		            if (status === 'complete' && result.info === 'OK') {
