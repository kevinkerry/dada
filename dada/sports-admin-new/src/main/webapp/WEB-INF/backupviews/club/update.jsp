...
<html>
    ...
    <body>
        <div class="container-fluid" style="margin-top: 30px;">
            <div class="row-fluid">
				<form id="club_form" class="form-horizontal"  action="${ctx}/club/update" method="post">
					<input type="hidden" name="clubId" value="${club.clubId}">
					<div class="form-group">
						<label class="col-md-1 control-label" >俱乐部名称：</label>
						<div class="col-md-2">
							<input class="form-control" name="clubName" type="text" value="${club.clubName}" required/>
						</div>
						<label class="control-label col-md-1" >俱乐部文化：</label>
						<div class="col-md-3">
							<input class="form-control" name="clubCulture" type="text" value="${club.clubCulture}" required/>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-md-1" >是否收费：</label>
						<c:if test="${club.feeFlag eq 1}">
							<div class="col-md-2">
								<label class="radio-inline">
								  <input type="radio" name="feeFlag" id="feeFlag1" value="1" required="required" checked="checked"/> 收费
								</label>
								<label class="radio-inline">
								  <input type="radio" name="feeFlag" id="feeFlag2" value="0" required="required"/> 不收费
								</label>
							</div>
							<label class="control-label col-md-1 feeComments">收费备注：</label>
							<div class="col-md-3 feeComments">
								<input class="form-control" name="feeComments" type="text" value="${club.feeComments}"/>
							</div>
						</c:if>
						<c:if test="${club.feeFlag eq 0}">
							<div class="col-md-2">
								<label class="radio-inline">
								  <input type="radio" name="feeFlag" id="feeFlag1" value="1" required="required"/> 收费
								</label>
								<label class="radio-inline">
								  <input type="radio" name="feeFlag" id="feeFlag2" value="0" required="required" checked="checked"/> 不收费
								</label>
							</div>
							<label class="control-label col-md-1">收费备注：</label>
							<div class="col-md-3">
								<input class="form-control" name="feeComments" type="text" readonly="readonly"/>
							</div>
						</c:if>
					</div>
					<div class="form-group">
						<label class="col-md-1 control-label" >俱乐部标志图：</label>
						<c:if test="${!empty club.clubLogo}">
							<div class="col-sm-1 btn-upload logo-upload" style="display: none;">
								<input id="logoupload" type="file" />
							</div>
							<div class="col-sm-1" id="logoimage">
								<div class="imgbox_logo" data-src="${club.clubLogo}" style="z-index:1;background-image: url('${image}${club.clubLogo}');">
					...
					<div class="form-group">
						...
						<div class="col-md-5">
							<textarea class="form-control" maxlength="200" name="clubDesc" style="resize: none;" rows="5">${club.clubDesc}</textarea>
						</div>
					</div> 
					<div class="form-group">
						<label class="control-label col-sm-1" >创建人</label>
						<div class="col-sm-2">
							<input class="form-control" name="clubCreator.userName" type="text" value="${club.clubCreator.userName}" readonly="readonly"/>
				        </div>
				        <label class="control-label col-sm-1" >持有者</label>
						<div class="col-sm-2">
							<input class="form-control" name="owner.userName" type="text" value="${club.owner.userName}" readonly="readonly"/>
				        </div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-1" >俱乐部级别</label>
						<div class="col-sm-2">
							<input class="form-control" name="clubLevel" type="text" value="${club.clubLevel}" readonly="readonly"/>
				        </div>
				        <label class="control-label col-sm-1" >活跃指数</label>
						<div class="col-sm-2">
							<input class="form-control" name="activeIndex" type="text" value="${club.activeIndex}" readonly="readonly"/>
				        </div>
					</div>
					<div class="form-group">
					...
					<input class="form-control" type="hidden" id="GPS_X" name="gpsX" placeholder="X" value="${club.gpsX}"/>
					<input class="form-control" type="hidden" id="GPS_Y" name="gpsY" placeholder="Y" value="${club.gpsY}"/>
		...
		<script src="${ctx }/resources/js/club/update.js"></script>
		<script >
			var ctx = '${ctx}';
			$(function() {
				new PCAS($('.form-horizontal #province')[0], $('.form-horizontal #city')[0],
					$('.form-horizontal #area')[0], '广东省', '${club.city}', '${club.district}');
				...
				$('#club_form input[name="feeFlag"]').on('change',function() {
					...
					if(feeFlag == 1) {
						...
						$('#club_form input[name="feeComments"]').val('');
				...
				$.ajax({
				   ...
				   success: function(msg){
					   ...
					   $.each( categories, function(i, category){
						   ...
						   }else if(category.parentCode == '${club.parentCategory}'){
			...
			function initForm() {
				$('#parent_category').val('${club.parentCategory}');
				$('#category_code').val('${club.categoryCode}');
