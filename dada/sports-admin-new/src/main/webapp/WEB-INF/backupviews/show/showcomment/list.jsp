    ...
    <body>
        <div class="container-fluid">
			<div class="form-group form-inline">	
				<legend>哒人姓名：${userName}</legend>
				<label style="float:left;font-size: 2rem;margin-top: 10px;">评论总数：${page.totalCount}</label>
				<div style="float:right;margin-bottom: 15px;">
		        	<button type="button" class="btn btn-default" onclick="javascript :location.href='${ctx}/show/list'"><i class=""></i>&nbsp;返回</button>
				</div>
		   </div>
...
<script>
	...
	function changeStatus(status, dom, event) {
		...
		location.href = "${ctx}/showcomment/changeStatus?commentId="+commentId+"&status="+status;
	}
	
	function goPage(num, pageSize) {
		var userName = encodeURI(encodeURI('${userName}'));
		location.href = "${ctx}/showcomment/list?currentPage="+num+"&pageSize="+pageSize+"&showId=${showId}&userName="+userName;
	}
</script>
