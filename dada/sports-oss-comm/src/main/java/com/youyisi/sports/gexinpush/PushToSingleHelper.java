...
public class PushToSingleHelper {
	...
	static Integer clientType = 2;
	// static String CID = "8c027398bca909789a1be75647887100";
	static String Alias = "";
	static String host = "http://sdk.open.api.igexin.com/apiex.htm";

	public static void push(final User user, final TransmissionContent param) {
		new Thread() {
			public void run() {
				try {
					...
					if (clientType == user.getClientType()) {
					...
					try {
						...
						System.out.println("正常：" + ret.getResponse().toString() + " 用户ID:" + user.getId() + " 个推ID:"
								+ user.getClientId());
	...
	public static TransmissionTemplate NotificationTemplateDemo(TransmissionContent param) throws Exception {
	...
	public static TransmissionTemplate TransmissionTemplateDemo(TransmissionContent param) throws Exception {
		TransmissionTemplate template = new TransmissionTemplate();
		template.setAppId(appId);
		template.setAppkey(appkey);
		template.setTransmissionType(2);
