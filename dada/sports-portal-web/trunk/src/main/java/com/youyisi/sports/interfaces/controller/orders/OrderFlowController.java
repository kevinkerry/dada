...
public class OrderFlowController extends BaseController{
	...
	private Logger log = LoggerFactory.getLogger(OrderFlowController.class);
	...
	public WebResultInfoWrapper list(Integer currentPage, Integer pageSize) {
		...
		webResultInfoWrapper.addResult("page", orderFlowServiceRemote.queryPage(page));
	...
	public WebResultInfoWrapper detail(@PathVariable("id") Long id) {
		WebResultInfoWrapper webResultInfoWrapper = new WebResultInfoWrapper();
		try {
		webResultInfoWrapper.addResult("orderFlow", orderFlowServiceRemote.get(id));
		} catch (SoaException e) {
		log.error("----message:"+e.getMessage());
		webResultInfoWrapper.setState(ERROR);
		webResultInfoWrapper.setMessage(SystemMessage.INTERFACEEXCEPTION_TEXT);
	...
	public WebResultInfoWrapper update(OrderFlow orderFlow) {
		WebResultInfoWrapper webResultInfoWrapper = new WebResultInfoWrapper();
		try {
		orderFlowServiceRemote.update(orderFlow);
		} catch (SoaException e) {
		log.error("----message:"+e.getMessage());
		webResultInfoWrapper.setState(ERROR);
		webResultInfoWrapper.setMessage(SystemMessage.INTERFACEEXCEPTION_TEXT);
