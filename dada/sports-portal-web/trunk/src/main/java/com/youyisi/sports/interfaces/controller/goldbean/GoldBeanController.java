...
public class GoldBeanController extends BaseController{

	@Autowired
	private GoldBeanServiceRemote goldBeanServiceRemote;

	private Logger log = LoggerFactory.getLogger(GoldBeanController.class);
	...
	public WebResultInfoWrapper list(Integer currentPage, Integer pageSize) {
		...
		webResultInfoWrapper.addResult("page", goldBeanServiceRemote.queryPage(page));
	...
	public WebResultInfoWrapper detail(@PathVariable("id") Long id) {
		WebResultInfoWrapper webResultInfoWrapper = new WebResultInfoWrapper();
		try {
		webResultInfoWrapper.addResult("goldBean", goldBeanServiceRemote.get(id));
		} catch (SoaException e) {
		log.error("----message:"+e.getMessage());
		webResultInfoWrapper.setState(ERROR);
		webResultInfoWrapper.setMessage(SystemMessage.INTERFACEEXCEPTION_TEXT);
	...
	public WebResultInfoWrapper update(GoldBean GoldBean) {
		WebResultInfoWrapper webResultInfoWrapper = new WebResultInfoWrapper();
		try {
		goldBeanServiceRemote.update(GoldBean);
		} catch (SoaException e) {
		log.error("----message:"+e.getMessage());
		webResultInfoWrapper.setState(ERROR);
		webResultInfoWrapper.setMessage(SystemMessage.INTERFACEEXCEPTION_TEXT);
